...
use noise::{Perlin, Fbm, NoiseModule, MultiFractal, Seedable};
...
impl Block {
    fn color(&self) -> [u8; 4] {
        match *self {
            Block::Air => [0x00, 0x00, 0x00, 0x00],
            Block::Stone => [0x70, 0x80, 0x90, 0xFF], // #708090 slate gray
            Block::Dirt => [0x80, 0x46, 0x1B, 0xFF], // #80461B russet brown
            Block::Grass => [0x0A, 0x85, 0x04, 0xFF], // #0a8504
            Block::Log => [0x4D, 0x33, 0x12, 0xFF], // #4d3312
            Block::Leaf => [0x17, 0x5C, 0x17, 0xFF], // #175c17
            Block::Water => [0x00, 0x57, 0xFF, 0xFF], // #0057ff
        }
    }
}
...
pub fn generate(pos: ChunkPosition, seed: usize) -> Arc<RwLock<Chunk>> {
    let mut output = Chunk::new();

    let height_noise = Fbm::new()
        .set_octaves(4)
        .set_seed(seed)
        .set_frequency(1.0/150.0);
    let dirt_noise = Perlin::new()
        .set_seed(seed);
    ...
    for x in 0..16i32 {
        for y in 0..16i32 {
            ...
            let height = {
                let noise = height_noise.get([xy_pos.0 as f32, xy_pos.1 as f32]);
            ...
            let dirt_height = {
                let scale = 32.0;
                let noise = dirt_noise.get([xy_pos.0 as f32 / scale, xy_pos.1 as f32 / scale]);
